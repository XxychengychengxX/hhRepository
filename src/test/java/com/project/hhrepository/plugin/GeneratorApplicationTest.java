 ...
 * @Date 2023/7/1
 */
package com.project.hhrepository.plugin;
...
public class GeneratorApplicationTest {
    ...
    private static final String databaseName = "hhrepository";
    ...
    protected static void simpleGenerator() {
        ...
        FastAutoGenerator.create(URL, USERNAME, PASSWORD)
                ...
                .strategyConfig((scanner, builder) -> builder.addInclude(getTables(scanner.apply(
                        ...
                        // 会在实体类的该字段上追加注解[@TableField(value = "create_time", fill = FieldFill
                        // .INSERT)]
                        //就是表项创建时间
                        .addTableFills(new Column("create_time", FieldFill.INSERT))
                        // 会在实体类的该字段上追加注解[@TableField(value = "update_time", fill = FieldFill
                        // .INSERT_UPDATE)]
    ...
    protected static void completeGenerator() {
        ...
        //   ---则生成system模块下的sys_user表，生成的实体文件将放在：com.yourdomain.projectname.entity
        //   .system包下，SysUser.java。
        // --- project-dao 的包路径：com.yourdomain.projectname.mapper，
        //   ---则生成system模块下的sys_user表，生成的mapper接口文件将放在：com.yourdomain.projectname.mapper
        //   .system包下，类名为：SysUserMapper.java。
        ...
        FastAutoGenerator.create(URL, USERNAME, PASSWORD)
                ...
                .strategyConfig((scanner, builder) -> builder.addInclude(getTables(scanner.apply(
                        ...
                        // 开启生成字段常量。
                        // 会在实体类末尾生成一系列 [public static final String NICKNAME = "nickname";]
                        // 的语句。（一般在写wapper时，会用到）
                        // .enableColumnConstant()
                        // 开启链式模型。
                        // 会在实体类前添加 [@Accessors(chain = true)] 注解。用法如 [User user=new User()
                        // .setAge(31).setName("snzl");]（这是Lombok的注解，需要添加Lombok依赖）
                        // .enableChainModel()
                        // 开启 lombok 模型。
                        ...
                        // 格式化 xml 实现类文件名称。
                        // 如果不设置，如表[sys_user]，默认的文件名是[SysUserMapper.xml]，写成下面这种形式后，将变成[SysUserXml
                        // .xml]。
